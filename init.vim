...
" Language support
Plug 'deoplete-plugins/deoplete-clang'		   " C/C++ auto completion
...
Plug 'lifepillar/pgsql.vim'                    " PostgreSQL syntax highlightingx
...
" ':TSInstall c cpp python go gomod gosum gowork java javascript typescript markdown css lua perl jq json php sql vim yaml tsx vue regex scss dart cmake bash make c_sharp'
...
" Go to define with split view
nnoremap 2gd :only<bar>vsplit<CR>gd
nnoremap 2gD :only<bar>vsplit<CR>gD
...
if (!exists('*SourceConfig'))
 function SourceConfig() abort
   ...
   source $MYVIMRC
   exe 'source ~/.config/nvim/lua/init.lua'
 endfunction
endif
...
" Set font for vimdev-icon
"set guifont=DroidSansM\ Nerd\ Font\:h10
...
lua require('dap-python').setup('~/.virtualenvs/debugpy/bin/python2')
lua require("nvim-dap-virtual-text").setup()
lua require('init')
lua require 'nvim-treesitter.install'.compilers = { 'clang' }

lua vim.fn.sign_define('DapBreakpoint',{ text ='🔴', texthl ='', linehl ='', numhl =''})
lua vim.fn.sign_define('DapStopped',{ text ='▶️', texthl ='', linehl ='', numhl =''})

"lua vim.keymap.set('n', '<F5>', require 'dap'.continue)
"lua vim.keymap.set('n', '<F8>', require 'dap'.step_over)
"lua vim.keymap.set('n', '<F9>', require 'dap'.step_into)
"lua vim.keymap.set('n', '<F10>', require 'dap'.step_out)
"lua vim.keymap.set('n', '<leader>bb', require 'dap'.toggle_breakpoint)
"lua vim.keymap.set('n', '<leader>br', require 'dap'.clear_breakpoints)

" Debug Adapter Protocol (DAP) Keybindings for Neovim
...
nnoremap <leader>bl :lua require'dap'.set_breakpoint(nil, nil, vim.fn.input('Log point message: '))<CR>
nnoremap <leader>br :lua require'dap'.clear_breakpoints()<CR>
nnoremap <leader>ba :Telescope dap list_breakpoints<CR>
...
" Variable inspection and widget commands
nnoremap <leader>di :lua require'dap.ui.widgets'.hover()<CR>
nnoremap <leader>d? :lua require('dap.ui.widgets').centered_float(require'dap.ui.widgets'.scopes)<CR>
nnoremap <leader>df :Telescope dap frames<CR>
nnoremap <leader>dh :Telescope dap commands<CR>

nnoremap <leader>vc :lua require'jdtls'.test_class()<CR>
nnoremap <leader>vm :lua require'jdtls'.test_nearest_method()<CR>

vnoremap <M-k> <Cmd>lua require("dapui").eval()<CR> " Alt+k
...
nnoremap <silent> <F4> :FloatermSend<CR>
...
"----------------------------------------------
" Mappings
au FileType java nmap <silent> gd <Plug>(coc-definition)
au FileType java nmap <silent> gy <Plug>(coc-type-definition)
au FileType java nmap <silent> gi <Plug>(coc-implementation)
au FileType java nmap <silent> gr <Plug>(coc-references)
au FileType java nmap <leader>qf <Plug>(coc-fix-current)
